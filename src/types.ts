...
export type BatchExpenseItem = Omit<
  Pick<TablesInsert<'expenses'>, 'category_id' | 'amount' | 'expense_date' | 'currency' | 'created_by_ai' | 'was_ai_suggestion_edited'>,
  'amount'
> & {
  amount: string;
};
 ...
 * Used in: POST /api/expenses/batch
 */
export type CreateExpenseBatchCommand = {
 ...
 * Used in: POST /api/expenses/batch
 */
export type BatchExpenseResponseDTO = {
  data: ExpenseDTO[];
  count: number;
};
 ...
 * Used in: POST /api/receipts/process
 */
export type ProcessReceiptResponseDTO = {
