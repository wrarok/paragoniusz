 ...
 */
function toDTO(dbExpense: DatabaseExpense): ExpenseDTO {
  if (!dbExpense.category) {
    throw new Error("Category is required for expense transformation");
  }
 ...
 */
function toDTOList(dbExpenses: DatabaseExpense[]): ExpenseDTO[] {
  return dbExpenses.map((expense) => toDTO(expense));
}
 ...
 */
function toInsertData(command: CreateExpenseCommand, userId: string): InsertExpense {
  if (!command.category_id || !command.amount || !command.expense_date) {
    throw new Error("Required fields missing in create expense command");
  }
 ...
 */
function toBatchInsertData(items: BatchExpenseItem[], userId: string): InsertExpense[] {
  return items.map((item) => {
    if (!item.category_id || !item.expense_date) {
      throw new Error("Required fields missing in batch expense item");
    }
 ...
 */
function toUpdateData(command: UpdateExpenseCommand): Partial<InsertExpense> {
...
/**
 * ExpenseTransformer object with all transformation methods
 */
export const ExpenseTransformer = {
  toDTO,
  toDTOList,
  toInsertData,
  toBatchInsertData,
