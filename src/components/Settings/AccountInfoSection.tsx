  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Zobacz szczegóły i metadane swojego konta</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          <div className="text-sm font-medium text-muted-foreground">ID użytkownika</div>
          <p className="mt-1 text-sm font-mono bg-muted px-3 py-2 rounded-md break-all">{profile.id}</p>
        </div>

        <div>
          <div className="text-sm font-medium text-muted-foreground">Data utworzenia konta</div>
          <p className="mt-1 text-sm">{formatDate(profile.created_at)}</p>
        </div>

        <div>
          <div className="text-sm font-medium text-muted-foreground">Ostatnia aktualizacja</div>
          <p className="mt-1 text-sm">{formatDate(profile.updated_at)}</p>
        </div>

        <div>
          <div className="text-sm font-medium text-muted-foreground">Status zgody na AI</div>
