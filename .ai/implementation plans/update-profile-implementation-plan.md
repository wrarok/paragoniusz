...
```typescript
export async function updateProfile(supabase: SupabaseClient<Database>, userId: string, data: UpdateProfileCommand) {
...
export const PATCH: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const updatedProfile = await updateProfile(locals.supabase, user.id, {
      userId: user.id,
