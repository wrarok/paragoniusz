...
async function fetchExpensesWithCategories(supabase: SupabaseClient, expenseIds: string[]): Promise<ExpenseDTO[]>;
```
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const createdExpenses = await createExpensesBatch(locals.supabase, user.id, expenses);
