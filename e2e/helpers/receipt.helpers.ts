...
export async function uploadReceipt(
  ...
  try {
    ...
    const uploadButton = await page.locator('button').filter({ hasText: /Prześlij|Przetwórz/ }).first();
...
export async function waitForAIProcessing(
  ...
  try {
    ...
    const hasAnyError = await page.isVisible('text=Wystąpił błąd')
...
export async function verifyExtractedData(
  ...
  if (!itemCount?.includes(expected.itemCount.toString())) {
    throw new Error(
      `Expected ${expected.itemCount} items, got ${itemCount}`
    );
  }
 ...
 * ```
 */
export async function editExpenseItem(
  page: Page,
  index: number,
  newAmount: string
): Promise<void> {
...
export async function giveAIConsent(page: Page): Promise<void> {
  ...
  const consentToggle = page.locator('input[type="checkbox"]').filter({ hasText: /AI|zgoda/i }).first();
 ...
 * ```
 */
export async function editMultipleItems(
  page: Page,
  edits: Array<{ index: number; amount: string }>
): Promise<void> {
