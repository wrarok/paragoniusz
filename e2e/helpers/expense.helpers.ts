...
export async function createExpense(page: Page, data: ExpenseData, skipNavigation = false): Promise<void> {
  ...
  await page
    .click('button:has-text("Wybierz kategorię")')
    .catch(() => page.locator('[role="combobox"]').first().click());
  ...
  await page.click('button:has-text("Dodaj wydatek")').catch(() => page.click('button[type="submit"]'));
