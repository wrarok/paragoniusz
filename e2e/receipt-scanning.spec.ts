...
test.describe('E2E: AI Receipt Scanning Journey', () => {
  ...
  test('User scans receipt, verifies data, saves expenses', async ({ page }) => {
    ...
    try {
      ...
      if (hasConsentMessage) {
        ...
        throw new Error(`AI scanning button not found. Available buttons: ${await page.$$eval('button', btns => btns.map(b => b.textContent?.trim()).filter(Boolean))}`);
      }
    }
  ...
  test('Should reject invalid file types', async ({ page }) => {
    ...
    const errorVisible = await verifyError(page, /Nieprawidłowy typ pliku|Prześlij tylko obrazy|akceptowane|image|jpg|png/i);
  ...
  test('Should require AI consent before processing', async ({ page }) => {
    ...
    const hasConsentDialog = await page.isVisible(
      'text=Wymagana zgoda na przetwarzanie AI',
      { timeout: 2000 }
    ).catch(() => false);
...
test.describe('Receipt Processing - Network and Error Handling', () => {
  ...
  test('Should handle file size limit exceeded', async ({ page }) => {
    ...
    const hasError = await verifyError(page, /za duży|rozmiar|limit/i)
      .catch(() => false);
