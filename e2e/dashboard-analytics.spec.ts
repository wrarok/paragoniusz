...
test.describe('E2E: Dashboard Analytics', () => {
  ...
  test('Dashboard displays correct analytics after multiple expenses', async ({ page }) => {
    ...
    expect(totalSpent).toBeGreaterThanOrEqual(225.00); // At least our 225
  ...
  test('Should show dashboard with expenses or empty state', async ({ page }) => {
    ...
    const hasEmptyState = await page.isVisible('text=Nie znaleziono wydatków')
...
test.describe('E2E: Dashboard Performance', () => {
  ...
  test('Should handle real-time updates', async ({ page }) => {
    ...
    expect(totalAfterFirst).toBeGreaterThanOrEqual(initialTotal + 100.00);
    ...
    expect(totalAfterSecond).toBeGreaterThanOrEqual(totalAfterFirst + 50.00);
  });
});