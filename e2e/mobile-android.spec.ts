...
test.describe('E2E: Mobile Android - User Experience', () => {
  ...
  test('Should display mobile-optimized navigation', async ({ page }) => {
    ...
    const hasMobileMenu = await page.isVisible('[data-testid="mobile-menu"]')
  ...
  test('Should display mobile-optimized dashboard', async ({ page }) => {
    ...
    if (percentageSmall > 30) {
      console.log(`⚠️  WARNING: ${percentageSmall.toFixed(1)}% of buttons are below minimum touch target size (44x44px)`);
  ...
  test('Should handle mobile scroll behavior', async ({ page }) => {
    ...
    const expenses = Array(5).fill(null).map((_, i) => ({
  ...
  test('Should handle pull-to-refresh gesture', async ({ page }) => {
    ...
    const isResponsive = await page.isVisible('h1')
      .catch(() => page.isVisible('[data-testid="dashboard-content"]'));
  ...
  test('Should handle Android back button', async ({ page }) => {
    ...
    await page.click('text=Zeskanuj paragon').catch(() => 
      page.goto('/expenses/scan')
    );
  ...
  test('Should display properly in dark mode', async ({ page }) => {
    ...
    const hasDarkModeToggle = await page.isVisible('text=Ciemny motyw')
