...
test.describe('E2E: Performance Tests', () => {
  ...
  test('Dashboard with multiple expenses should render efficiently', async ({ page }) => {
    ...
    const createResult = await page.evaluate(async () => {
      try {
        const expenses = Array(10).fill(null).map((_, i) => ({
  ...
  test('Chart rendering should not block UI', async ({ page }) => {
    ...
    const hasChart = await page.isVisible('canvas[data-testid="expense-chart"]')
  ...
  test('Large file upload should show progress', async ({ page }) => {
    ...
    await page.setInputFiles(
      'input[type="file"]',
      './e2e/fixtures/receipts/slow-receipt.jpg'
    );
    
    // Progress indicator should appear
    const hasProgress = await page.isVisible('[role="progressbar"]')
