...
import type { ProcessingContext, ProcessingStep } from "../../../src/lib/processing/receipt-processing-steps";
...
describe("ReceiptService - Integration Tests", () => {
  ...
  describe("uploadReceipt()", () => {
    it("should upload file to storage successfully", async () => {
      ...
      expect(mockUpload).toHaveBeenCalledWith(`receipts/${userId}/${mockUUID}.jpg`, expect.any(ArrayBuffer), {
  ...
  describe("processReceipt() - Pipeline Integration", () => {
    it("should execute all steps in pipeline and return result", async () => {
      ...
      expect(result).toEqual({
        expenses: expect.arrayContaining([
          expect.objectContaining({
            ...
            amount: "9.70",
            items: expect.arrayContaining(["Chleb - 5.50", "Mleko - 4.20"]),
          }),
        ]),
        total_amount: "9.70",
        currency: "PLN",
      ...
      expect(mockSupabase.functions.invoke).toHaveBeenCalledWith("process-receipt", expect.any(Object));
    });
    ...
    it("should propagate error from ConsentValidationStep", async () => {
      ...
      // Act & Assert
      await expect(service.processReceipt(filePath, userId)).rejects.toThrow("AI_CONSENT_REQUIRED");
    });
    ...
    it("should propagate error from FileOwnershipValidationStep", async () => {
      ...
      // Act & Assert
      await expect(service.processReceipt(filePath, userId)).rejects.toThrow("FORBIDDEN");
    });
    ...
    it("should propagate error from CategoryFetchStep", async () => {
      ...
      // Act & Assert
      await expect(service.processReceipt(filePath, userId)).rejects.toThrow("Nie udało się pobrać kategorii");
    });
    ...
    it("should propagate error from AIProcessingStep", async () => {
      ...
      // Act & Assert
      await expect(service.processReceipt(filePath, userId)).rejects.toThrow("RATE_LIMIT_EXCEEDED");
    });
  ...
  describe("Error Handling", () => {
    it("should throw error if pipeline produces no result", async () => {
      ...
      // Act & Assert
      await expect(service.processReceipt("receipts/user-123/test.jpg", "user-123")).rejects.toThrow(
        "Pipeline failed to produce result"
      );
    });
  });
});
