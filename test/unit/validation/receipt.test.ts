...
describe('Receipt Validation', () => {
  describe('uploadReceiptSchema', () => {
    ...
    it('should reject empty file', () => {
      ...
      expect(() => uploadReceiptSchema.parse({ file })).toThrow('File cannot be empty');
    });

    it('should reject unsupported file types', () => {
      const pdfFile = new File(['content'], 'receipt.pdf', { type: 'application/pdf' });
      expect(() => uploadReceiptSchema.parse({ file: pdfFile })).toThrow(
        'File must be JPEG, PNG, or HEIC format'
      );

      const txtFile = new File(['content'], 'receipt.txt', { type: 'text/plain' });
      expect(() => uploadReceiptSchema.parse({ file: txtFile })).toThrow(
        'File must be JPEG, PNG, or HEIC format'
      );
    });
  });
  ...
  describe('processReceiptSchema', () => {
    ...
    it('should reject empty file path', () => {
      expect(() => processReceiptSchema.parse({ file_path: '' })).toThrow(
  ...
  describe('receiptDataSchema', () => {
    it('should accept valid receipt data', () => {
      const validData = {
        items: [
          ...
          { name: 'Chleb', amount: 3.20, category: 'żywność' },
        ],
        total: 8.70,
        date: '2024-01-15',
      };
    ...
    it('should reject receipt data with empty items array', () => {
      ...
      expect(() => receiptDataSchema.parse(invalidData)).toThrow(
    ...
    it('should reject receipt data with negative total', () => {
      const invalidData = {
        items: [{ name: 'Mleko', amount: 5.50, category: 'żywność' }],
        total: -5.50,
        date: '2024-01-15',
      };
