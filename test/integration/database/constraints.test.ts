    ...
    it('should reject future dates', async () => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          category_id: categoryId,
          amount: 50.00,
          expense_date: tomorrowStr, // Future date
          currency: 'PLN',
        })
        .select()
        .single();
    ...
    it('should reject amounts exceeding max value', async () => {
      const { data, error } = await supabase
        ...
        .insert({
          user_id: TEST_USER.id,
          category_id: categoryId,
          amount: 100000000.00, // Exceeds numeric(10,2)
          expense_date: '2024-01-15',
          currency: 'PLN',
        })
        .select()
      ...
      expect(error).not.toBeNull();
      expect(data).toBeNull();
      expect(error?.message).toContain('numeric field overflow');
    });
    ...
    it('should fail if any constraint is violated', async () => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          user_id: TEST_USER.id,
          category_id: categoryId,
          amount: -10.00, // Violates amount constraint
          expense_date: tomorrow.toISOString().split('T')[0], // Violates date constraint
          currency: 'PLN',
        })
        .select()
        .single();
      ...
      expect(error).not.toBeNull();
      expect(data).toBeNull();
      // Should catch the first constraint violation
      expect(error?.message).toBeDefined();
    });
  });
});