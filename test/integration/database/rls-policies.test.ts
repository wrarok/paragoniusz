...
import { cleanTestDataWithClient, cleanTestDataUserB, createTestExpense, getCategoryByName } from '../../helpers/test-database';
...
describe('RLS Policies - Expenses Table', () => {
  ...
  describe('SELECT Policy - Read Isolation', () => {
    it('should allow user A to read their own expenses', async () => {
    ...
    it('should prevent user A from reading user B expenses', async () => {
      const supabaseA = await createAuthenticatedClient();
    ...
    it('should return empty array when user A queries all expenses but only user B has data', async () => {
      const supabaseA = await createAuthenticatedClient();
      const supabaseB = await createClientWithUser(TEST_USER_B.email, TEST_USER_B.password);
  ...
  describe('INSERT Policy - Write Isolation', () => {
    it('should allow user A to insert their own expenses', async () => {
      const supabaseA = await createAuthenticatedClient();
    ...
    it('should prevent user A from inserting expenses for user B', async () => {
      const supabaseA = await createAuthenticatedClient();
  ...
  describe('UPDATE Policy - Modify Isolation', () => {
    it('should allow user A to update their own expenses', async () => {
      const supabaseA = await createAuthenticatedClient();
    ...
    it('should prevent user A from updating user B expenses', async () => {
      const supabaseA = await createAuthenticatedClient();
      const supabaseB = await createClientWithUser(TEST_USER_B.email, TEST_USER_B.password);
    ...
    it('should prevent user A from changing user_id to user B in update', async () => {
      const supabaseA = await createAuthenticatedClient();
  ...
  describe('DELETE Policy - Remove Isolation', () => {
    it('should allow user A to delete their own expenses', async () => {
      const supabaseA = await createAuthenticatedClient();
      ...
      // Verify deletion
      const { data: checkData } = await supabaseA
        .from('expenses')
        .select('*')
        .eq('id', expense.id)
        .single();
    ...
    it('should prevent user A from deleting user B expenses', async () => {
      const supabaseA = await createAuthenticatedClient();
      const supabaseB = await createClientWithUser(TEST_USER_B.email, TEST_USER_B.password);
      ...
      const { data: checkData } = await supabaseB
        .from('expenses')
        .select('*')
        .eq('id', expenseB.id)
        .single();
  ...
  describe('Multi-User Scenarios', () => {
    it('should correctly isolate data when both users have expenses', async () => {
      // Create expenses for both users
      await createTestExpense(supabaseA, {
        category_id: categoryId,
        amount: '10.00',
